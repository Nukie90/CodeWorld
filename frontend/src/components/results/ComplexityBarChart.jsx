...
function ComplexityBarChart({ files = [] }) {
  const prepared = useMemo(() => {
    const maxNloc = Math.max(
      ...files.map((file) => file.total_nloc || file.total_loc || 0),
      1
    );
  ...
  return (
    <div
      style={{
        width: `${prepared.svgWidth}px`,
        display: "inline-block",
        verticalAlign: "top",
        height: `${CHART_HEIGHT + LABEL_AREA + 100}px`,
      }}
    >
      <svg
        role="img"
        ...
        height={CHART_HEIGHT + LABEL_AREA + 100}
      ...
      >
        {prepared.bars.map((bar, index) => (
          <g
            key={bar.label || index}
            transform={`translate(${bar.x}, ${CHART_HEIGHT - bar.height})`}
          >
            <rect
              width={bar.width}
              height={bar.height}
